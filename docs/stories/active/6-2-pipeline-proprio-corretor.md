...
## Acceptance Criteria
- [ ] AC1: Pagina `/broker/pipeline` renderiza kanban identico ao admin (reusar componentes da Story 4.1)
- [ ] AC2: Leads filtrados por `assigned_broker_id = currentUser.id` — corretor so ve seus leads
- [ ] AC3: Drag-and-drop funcional (mover lead entre etapas)
- [ ] AC4: Ao mover, registra activity log com `created_by = broker_id`
- [ ] AC5: Card do lead exibe: nome, empreendimento, tempo na etapa, score, ultima mensagem (truncada)
- [ ] AC6: Card NAO exibe corretor (redundante — todos sao do corretor logado)
- [ ] AC7: Contagem de leads por etapa (filtrada)
- [ ] AC8: Realtime: novo lead designado aparece automaticamente
- [ ] AC9: Clicar em card navega para `/broker/leads/[id]` (detalhe do lead versao corretor — Story 6.4)
- [ ] AC10: Se corretor nao tem leads, empty state: "Voce nao tem leads designados. Novos leads serao atribuidos pelo supervisor."
