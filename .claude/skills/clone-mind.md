...
5. **Invoke @identity-analyst** 🔴 HARD CHECKPOINT — SYNTHESIS IS GATED
   - Task: Extract identity core (L6-L8)
   - Output: `outputs/minds/{slug}/analysis/identity-core.yaml`

   **APÓS @identity-analyst completar — OBRIGATÓRIO antes de qualquer próximo passo:**

   a. Leia `outputs/minds/{slug}/analysis/identity-core.yaml`

   b. Use `AskUserQuestion` apresentando os 3 elementos:
      - L6: Hierarquia de Valores extraída
      - L7: Obsessões identificadas
      - L8: Contradições produtivas mapeadas
      - Opções: **APPROVE** / **REVISE** / **ABORT**

   c. Se **APPROVE**: escreva o sentinel file:
      ```yaml
      # outputs/minds/{slug}/analysis/identity-core-approved.yaml
      approved: true
      approved_at: {timestamp ISO8601}
      slug: {slug}
      layers_approved: [L6, L7, L8]
      notes: ""
      ```

   d. Se **REVISE**: re-execute @identity-analyst com o feedback do usuário. Retorne ao passo (a).

   e. Se **ABORT**: escreva `outputs/minds/{slug}/metadata/pipeline_state.yaml` com `status: aborted`. Pare.

   > ⚠️ O hook `mind-clone-governance.py` bloqueia qualquer escrita em
   > `outputs/minds/{slug}/synthesis/` enquanto `identity-core-approved.yaml`
   > não existir. Não há como bypassar esse gate.
