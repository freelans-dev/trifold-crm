...
ALLOWED_COMMANDS = [
    ...
    "supabase db reset",               # Reset controlado
    "supabase db diff",                # Diff de schema (leitura)
    "pg_dump",                         # Backup (exportar)
    "psql.*-f.*migrations",            # Aplicar migration file (legado)
    "psql.*-f",                        # Aplicar qualquer SQL file — mais seguro que DDL inline
    # db-sage: agente autônomo com governança própria (Section 5).
    # Usa supabase CLI e file-based SQL. DDL inline via -c ainda é bloqueado
    # para evitar execução não-documentada fora do fluxo de task files.
]
